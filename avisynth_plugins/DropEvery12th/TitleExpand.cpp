// TitleExpand.cpp : Defines the entry point for the DLL application.
//
...
public:
  ...
  // Otherwise they can only be called from functions within the class itself.
	...
	TitleExpand(PClip _child,  int frame, IScriptEnvironment* env);
...
TitleExpand::TitleExpand(PClip _child, int frame, IScriptEnvironment* env) :
	GenericVideoFilter(_child),
	dframe(frame)
{
	if (!vi.IsYV12() && !vi.IsRGB32() &&! vi.IsYUY2())
		env->ThrowError("TitleExpand : RGB32, YUY2 and YV12 are only supported");

	if (!vi.IsYV12() && ((ex_left|ex_right|ex_top|ex_bottom) & 1) != 0)
		env->ThrowError("TitleExpand : every margin must be even");

	if (!vi.IsYUY2() && ((ex_left|ex_right) & 1) != 0)
		env->ThrowError("TitleExpand : left and right margins must be even");

	vi.width += ex_left+ex_right;
	vi.height += ex_top+ex_bottom;
}
...
PVideoFrame __stdcall TitleExpand::GetFrame(int n, IScriptEnvironment* env)
{
...
h  0  1   2    3
  sp sp sp+1 sp+1
      +       
*/
			if (h >= ex_top && h < ex_top+src_height-1)
	...
	if (vi.IsYV12())
	{
		...
		int ex_leftUV = ex_left/2;
		int ex_topUV = ex_top/2;

		for (int uv = 0; uv < 2; ++uv)
		{
			for (int h = 0; h < dst_heightUV; ++h)
			{
				for (int w = 0; w < dst_widthUV; ++w)
				{
					int sw = w - ex_leftUV;
					if (sw < 0)
						sw = 0;
				...
				if (h >= ex_topUV && h < ex_topUV+src_heightUV-1)
					srcp = srcp + src_pitchUV; // Add the pitch (note use of pitch and not width) of one line (in bytes) to the source pointer

				dstp = dstp + dst_pitchUV; // Add the pitch to the destination pointer.
			}
		...
		for (int h = 0; h < dst_height; ++h)
		{
			for (int w = 0; w < dst_width/4; ++w)
			{
				...
				for (int i = 0; i < 4; ++i)
				{
					dstp[w*4 + i] = srcp[sw*4 + i];
					if (l)
						dstp[w*4+0] = dstp[w*4+2];
					if (r)
						dstp[w*4+2] = dstp[w*4+0];
				}
			}															
			if (h >= ex_top && h < ex_top+src_height-1)
				srcp = srcp + src_pitch; // Add the pitch (note use of pitch and not width) of one line (in bytes) to the source pointer
	...
	// This version of TitleExpand is intended to show how to utilise information from 2 clips in YUY2
